...
class AdfsBaseBackend(ModelBackend):
    def exchange_auth_code(self, authorization_code, request):
        ...
        response = provider_config.session.post(
            provider_config.token_endpoint, data, timeout=settings.TIMEOUT
        )
    ...
    def get_group_memberships_from_ms_graph(self, obo_access_token):
        ...
        response = provider_config.session.get(
            graph_url, headers=headers, timeout=settings.TIMEOUT
        )
        # 200 = valid token received
        # 400 = 'something' is wrong in our request
    ...
    def process_access_token(self, access_token, adfs_response=None):
        ...
        print("claaa", claims)
        if settings.USERNAME_CLAIM in claims:
            claims[settings.USERNAME_CLAIM] = claims[settings.USERNAME_CLAIM].lower()
        elif settings.GUEST_USERNAME_CLAIM in claims:
            claims[settings.GUEST_USERNAME_CLAIM] = claims[
                settings.GUEST_USERNAME_CLAIM
            ].lower()
...
class AdfsAuthCodeBackend(AdfsBaseBackend):
    ...
    def authenticate(self, request=None, authorization_code=None, **kwargs):
        ...
        UserLoginDetail.objects.create(
            user=user, activity_type=UserActionChoices.LOGIN.value
        )
        return user
