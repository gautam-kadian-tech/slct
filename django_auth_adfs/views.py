    ...
    def get(self, request):
        ...
        if user:
            if user.is_active:
                ...
                print("hereeee", json.dumps(list(user_roles), cls=DjangoJSONEncoder))
                redirect_url = django_settings.SSO_LOGIN_REDIRECT_URL
                return settings.CUSTOM_SUCCESS_RESPONSE_VIEW(
                    request,
                    message="Redirecting",
                    access_token="Token " + str(user.get_auth_token()),
                    user_persona=json.dumps(list(user_roles), cls=DjangoJSONEncoder),
